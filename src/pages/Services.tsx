        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {services.map((service, index) => (
            <Link to={`/service/${service.id}`} key={index} className="group relative border-b border-r border-white/10 p-8 lg:p-12 min-h-[320px] flex flex-col justify-center bg-navy hover:bg-[#6e6e70] transition-colors duration-500 cursor-pointer overflow-hidden block">
              <div className="absolute top-6 right-6 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 transform md:translate-y-2 md:group-hover:translate-y-0">
                <ArrowUpRight className="w-6 h-6 text-white" />
              </div>
              <h3 className="font-serif text-2xl lg:text-3xl mb-4 relative z-10">{service.title}</h3>
              <div className="h-auto opacity-100 md:h-0 md:opacity-0 md:group-hover:h-auto md:group-hover:opacity-100 transition-all duration-500 overflow-hidden">
